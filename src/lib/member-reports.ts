...
export interface MemberInvestmentSummary {
  ...
  /** Net balance = totalPrincipal + totalRoi − totalWithdrawals (Total Investment + Total ROI − Total Withdrawals). This is the correct "Account Balance" figure. */
  netBalance: number;
...
export function summarizeMemberReports(rows: MemberReportRow[]): MemberInvestmentSummary {
  ...
  if (!latestRow) {
    return {
      ...
      totalWithdrawals,
      totalInvested: 0,
      netBalance: totalPrincipal + totalRoi - totalWithdrawals,
      latestClosingBalance: null,
      latestRow: rows[0],
      hasReports: true,
    };
